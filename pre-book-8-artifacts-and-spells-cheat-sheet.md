# Pre-Book-8 Artifacts And Spells Cheat Sheet

This is a spoiler-safe lookup sheet for artifacts, named spells, class-linked items, deity-linked gear, and recurring mechanics that book 8 references or assumes you will recognize.

It is deliberately more exhaustive than the other recap files and is organized alphabetically so you can use it mid-read.

## How To Read It

- "First major appearance" means the first book where the item or concept becomes materially important, not necessarily the first offhand mention.
- "Why it matters" is aimed at recall, not full mechanical detail.
- If an entry sounds only half familiar, that usually means it belongs to one of four recurring buckets: Carl's burden-powered gear, Donut's title or class packages, divine relics, or transformation/control mechanics such as pet biscuits.

## Mini Index

### Artifacts And Gear

- [`Bolt of Ophiotaurus`](#bolt-of-ophiotaurus)
- [`Court of Sekhmet Headdress`](#court-of-sekhmet-headdress)
- [`Danforth`](#danforth)
- [`Emberus Ring`](#emberus-ring)
- [`Gate of the Feral Gods`](#gate-of-the-feral-gods)
- [`Kneepads of the Munificent Goddess Kina`](#kneepads-of-the-munificent-goddess-kina)
- [`Memorial Crystal`](#memorial-crystal)
- [`Memorial Crystal: Apito`](#memorial-crystal-apito)
- [`Memorial Crystal: Emberus`](#memorial-crystal-emberus)
- [`Memorial Crystal: Geyrun`](#memorial-crystal-geyrun)
- [`Memorial Crystal: Ysalte`](#memorial-crystal-ysalte)
- [`Petey Motteux's Little Secret`](#petey-motteuxs-little-secret)
- [`Ring of Divine Suffering`](#ring-of-divine-suffering)
- [`Scorpion's Bite`](#scorpions-bite)
- [`Wrap of Sekhmet's Compassion`](#wrap-of-sekhmets-compassion)

### Spells And Skills

- [`Astral Paw`](#astral-paw)
- [`Bad Attitude`](#bad-attitude)
- [`Cockroach Skill`](#cockroach-skill)
- [`Escape Plan`](#escape-plan)
- [`Examine Portal`](#examine-portal)
- [`Find Traps`](#find-traps)
- [`Find Weakened`](#find-weakened)
- [`Gatekeeper Spell`](#gatekeeper-spell)
- [`Gloom Wraith Phase`](#gloom-wraith-phase)
- [`Heal Critter`](#heal-critter)
- [`Healer's Grasp`](#healers-grasp)
- [`Hole Spell`](#hole-spell)
- [`Ice Slick`](#ice-slick)
- [`Launch`](#launch)
- [`Love Vampire`](#love-vampire)
- [`Mind Balance`](#mind-balance)
- [`Oozy Form`](#oozy-form)
- [`Puddle Jumper`](#puddle-jumper)
- [`Radar`](#radar)
- [`Shield Spell`](#shield-spell)
- [`Smoke Form`](#smoke-form)
- [`Split Personality`](#split-personality)
- [`Steady Charge`](#steady-charge)
- [`Torch`](#torch)
- [`Transformation Spell`](#transformation-spell)
- [`Twinkle Toes`](#twinkle-toes)
- [`Wall of Fire`](#wall-of-fire)

### Biscuits And Transformations

- [`Enhanced Pet Biscuits`](#enhanced-pet-biscuits)
- [`Nothing Special`](#nothing-special)
- [`Nothing Special Party Companion`](#nothing-special-party-companion)
- [`Paramour`](#paramour)
- [`Pet Biscuits`](#pet-biscuits)

### Gods And Divine Context

- [`Apito`](#apito)
- [`Emberus`](#emberus)
- [`Geyrun`](#geyrun)
- [`Khepri`](#khepri)
- [`Nekhebit`](#nekhebit)
- [`Sekhmet`](#sekhmet)
- [`Ysalte`](#ysalte)

### Class Packages And Reward Categories

- [`Assassin of Sekhmet`](#assassin-of-sekhmet)
- [`Celestial Boon`](#celestial-boon)
- [`Requite Dahlia`](#requite-dahlia)
- [`Spellbook Of The Floor`](#spellbook-of-the-floor)

### Tactical Consumables And Utility Tools

- [`Banger Spheres`](#banger-spheres)
- [`Emergency Gremlin Scrolls`](#emergency-gremlin-scrolls)
- [`Source Scroll / Water Source Scroll`](#source-scroll--water-source-scroll)

## Alphabetical Reference

### Apito

- Type: goddess / myth-anchor / memorial-crystal context.
- First major appearance: books 2 and 5, with much greater importance in books 6 and 7.
- What to remember: Apito sits near the center of the old divine mythology. References to Apito often point toward distorted history, divine family structures, old religious narratives, and the memorial crystal mystery.
- Why it matters in book 8 context: if an item, crystal, or myth is tied to Apito, it is usually part of a much larger theological puzzle.

### Assassin of Sekhmet

- Type: Donut class package.
- First major appearance: book 8.
- What to remember: this is not just a stat choice. It is a whole themed package with deity-linked flavor, pet accessories, mobility/phase options, support effects, and social consequences.
- Why it matters: many of the book-8 item and skill names tied to Donut come from this package.

### Astral Paw

- Type: Donut spell.
- First major appearance: book 4.
- What to remember: a claw-oriented cutting or striking spell that sounds decorative but is often used as serious offense or utility.
- Why it matters: by later books it is part of Donut's established "cute presentation, brutal function" toolkit.

### Bad Attitude

- Type: Donut spell.
- First major appearance: pre-book-8, earlier than book 8; explicitly discussed again in book 8.
- What to remember: a crowd-chaos or disruption-oriented spell rather than straightforward single-target damage.
- Why it matters: book 8 treats it as one of Donut's mature, known options rather than new tech.

### Banger Spheres

- Type: throwable delivery tool.
- First major appearance: later pre-book-8 era.
- What to remember: Carl uses these as precision delivery systems for explosives or other payloads.
- Why it matters: they are classic Carl tools, useful because they let him put exactly the wrong thing in exactly the right place.

### Bolt of Ophiotaurus

- Type: named mythic bolt.
- First major appearance: book 3.
- What to remember: this is not ordinary ammunition. It belongs to the category of named, high-leverage bolts meant for impossible targets or set-piece moments.
- Why it matters: if this bolt is being used, the scene has probably escalated beyond normal combat assumptions.

### Celestial Boon

- Type: permanent divine-style reward category.
- First major appearance: pre-book-8, explicitly invoked again in book 8.
- What to remember: a celestial boon is more than a temporary buff or ordinary skill gain. It usually means a lasting change in capability or status.
- Why it matters: book 8 references it to distinguish certain rewards from normal skills or spells.

### Cockroach Skill

- Type: Carl survival skill.
- First major appearance: pre-book-8.
- What to remember: one of Carl's defining endurance-survival mechanics.
- Why it matters: when it comes up, it usually signals Carl's build philosophy: stay alive through stubborn, ugly persistence.

### Court of Sekhmet Headdress

- Type: Donut class-linked item.
- First major appearance: book 8.
- What to remember: a feline-only headdress from the Sekhmet package that grants offensive or binding-spell warning.
- Why it matters: it reinforces Donut's battlefield awareness and divine alignment, and it comes with deity-political downsides as well as benefits.

### Danforth

- Type: enchanted crossbow bolt.
- First major appearance: book 8.
- What to remember: a rooting or anchoring bolt keyed to Crossbow skill.
- Why it matters: it is control ammunition, not just damage ammunition. If Donut is using Danforth bolts, the immediate goal is usually to hold something in place long enough for a larger plan to work.

### Deflector

- Type: defensive ability.
- First major appearance: referenced in book 8.
- What to remember: part of the broader field of non-glamorous race-combat utility abilities.
- Why it matters: included here because book 8's race and vehicle encounters repeatedly rely on practical utility, not only flashy signature magic.

### Emergency Gremlin Scrolls

- Type: summoning consumable.
- First major appearance: book 8.
- What to remember: these bring in temporary engineering help and fit perfectly into Carl's "solve a combat problem with logistics" mindset.
- Why it matters: they become one of the clearer examples of book 8's engineering-comedy-toolkit turning into real tactical leverage.

### Emberus

- Type: god / quest anchor / ring context.
- First major appearance: book 4, with greater importance in books 6 and 7.
- What to remember: Emberus is one of Carl's most consequential divine entanglements.
- Why it matters: if a ring, quest, bargain, or threat is Emberus-linked, assume it has god-level consequences rather than ordinary dungeon stakes.

### Emberus Ring

- Type: divine ring.
- First major appearance: book 7.
- What to remember: a god-linked ring that acts as access key, burden, lever, and liability.
- Why it matters: this is one of the clearest signs that Carl is operating with divine tools that are as dangerous as they are useful.

### Enhanced Pet Biscuits

- Type: transformation item family.
- First major appearance: book 1.
- What to remember: these are stronger, stranger, and more consequential than ordinary pet biscuits. They can alter identity, dependency, race-track status, and long-term role.
- Why it matters: several of the biscuit variants mentioned in book 8 only make sense if you remember that enhanced biscuits can permanently change the board.

### Escape Plan

- Type: Carl utility skill.
- First major appearance: pre-book-8.
- What to remember: this is a contingency-awareness skill, not an offensive showpiece.
- Why it matters: it captures the recurring Carl pattern of surviving through preparation, inference, and weird practical caution.

### Examine Portal

- Type: utility/detection spell or skill.
- First major appearance: book 8 reference.
- What to remember: one of the information-gathering tools that matters because Carl's side survives by understanding systems better than other people do.
- Why it matters: in this series, utility examination skills are often plot-driving.

### Find Traps

- Type: utility skill.
- First major appearance: pre-book-8; heavily leaned on in book 8.
- What to remember: a classic Carl information skill.
- Why it matters: it is representative of the whole "detection beats brute force" side of the series.

### Find Weakened

- Type: utility/combat-reading skill.
- First major appearance: pre-book-8; explicitly advanced in book 8.
- What to remember: one of Carl and Donut's tools for turning observation into tactical advantage.
- Why it matters: another example of why their side performs above raw-stat expectation.

### Gate of the Feral Gods

- Type: major artifact.
- First major appearance: book 4.
- What to remember: one of the single most important artifacts in the series. It affects movement, positioning, access, and escalation in ways normal items do not.
- Why it matters: later books treat control of the Gate as a real political and military issue.

### Gatekeeper Spell

- Type: Donut spell from a spellbook.
- First major appearance: book 8.
- What to remember: an anti-class or anti-deity interference spell keyed to a specific spell target setup.
- Why it matters: it belongs to the class of effects that are dangerous not because they do damage, but because they strip targets of key abilities.

### Geyrun

- Type: dead god / memorial-crystal and quest context.
- First major appearance: book 6.
- What to remember: Geyrun's murder becomes a major divine investigation thread.
- Why it matters: if Geyrun is mentioned, it is usually in connection with corruption in the old divine order.

### Gloom Wraith Phase

- Type: Carl phase/mobility skill.
- First major appearance: book 7.
- What to remember: a major late-series Carl movement skill that lets him ignore normal geometry in very Carl ways.
- Why it matters: once Carl has this, line of sight, barriers, and body-position assumptions become much less reliable.

### Heal Critter

- Type: Donut support spell.
- First major appearance: book 1.
- What to remember: a longstanding pet/support spell in Donut's kit.
- Why it matters: it is one of the oldest reminders that Donut's build has always had real support capacity.

### Healer's Grasp

- Type: aura/support effect.
- First major appearance: book 8.
- What to remember: one of the support-oriented benefits tied to Donut's Sekhmet package.
- Why it matters: it shows that Donut's later class packages are field-wide support suites, not narrow caster upgrades.

### Hole Spell

- Type: Donut signature utility spell.
- First major appearance: book 3.
- What to remember: one of Donut's oldest and most abusable spells, repeatedly used for traversal, breach, escape, and rule-bending.
- Why it matters: this is one of the clearest examples of the series rewarding lateral thinking over intended-use play.

### Ice Slick

- Type: control spell.
- First major appearance: book 8 as a newly acquired spellbook, though the spell existed earlier in Elle's toolkit.
- What to remember: battlefield terrain control through slick ice.
- Why it matters: it is practical, not flashy, which is exactly why it keeps being useful.

### Khepri

- Type: god / temple thread / later combat context.
- First major appearance: book 7.
- What to remember: one of the gods book 8 expects you to recognize immediately.
- Why it matters: Khepri references usually point back to the temple sequence, divine marking, and the broader unraveling of the divine order.

### Kneepads of the Munificent Goddess Kina

- Type: enchanted divine-flavored gear.
- First major appearance: pre-book-8; specifically linked back to Katia in book 8.
- What to remember: originally gifted to Katia.
- Why it matters: practical item, but also one of the pieces of lingering Katia-linked gear still shaping the team's options after her departure.

### Launch

- Type: mobility/impact skill.
- First major appearance: referenced in book 8.
- What to remember: a straightforward but potent positional tool.
- Why it matters: another reminder that book 8's action logic often depends on stacked practical mobility tech.

### Li Na Chain

- Type: named move/chain effect associated with Donut and Li Na.
- First major appearance: book 7.
- What to remember: one of the important late-Faction-Wars moves with lasting consequences.
- Why it matters: if this is mentioned, it is usually as shorthand for a pivotal late-book-7 event and its aftermath.

### Love Vampire

- Type: Donut charm/drain-oriented skill.
- First major appearance: pre-book-8; explicitly referenced in book 8.
- What to remember: an example of ability theft, drain, or affinity-style mechanics.
- Why it matters: book 8 invokes it as a model while discussing domination and mind-affecting mechanics.

### Memorial Crystal

- Type: divine relic category.
- First major appearance: book 5, with much larger importance in books 6 and 7.
- What to remember: formed from dead gods, carrying memory, spell-content, or divine trace.
- Why it matters: never treat one as ordinary loot. They are evidence, theology, power, and quest fuel in one package.

### Memorial Crystal: Apito

- Type: specific memorial crystal.
- First major appearance: books 6 and 7.
- What to remember: a major mystery because Apito is not supposed to be dead.
- Why it matters: this is one of the most important crystal references for understanding the instability of the old stories.

### Memorial Crystal: Emberus

- Type: specific memorial crystal.
- First major appearance: book 8.
- What to remember: part of the way book 8 keeps escalating the crystal logic into active present-tense danger.
- Why it matters: reinforces that memorial crystals are not just retrospective relics.

### Memorial Crystal: Geyrun

- Type: specific divine-death context.
- First major appearance: book 6.
- What to remember: tied to Carl's divine investigation and to Emberus's personal stakes.
- Why it matters: a useful anchor for the whole "dead gods leave actionable residue" pattern.

### Memorial Crystal: Ysalte

- Type: specific memorial crystal.
- First major appearance: book 6.
- What to remember: appears after Ysalte's death.
- Why it matters: one of the clearest examples showing how the crystal system works.

### Mind Balance

- Type: resistance/mental defense skill.
- First major appearance: pre-book-8; explicitly referenced in book 8.
- What to remember: protects against at least some hostile mind-affecting abilities.
- Why it matters: book 8 repeatedly plays with domination, split identity, and coercive magical effects.

### Nekhebit

- Type: goddess / title anchor / Donut patron context.
- First major appearance: books 3 and 6, with major escalation in book 7.
- What to remember: if Nekhebit is mentioned, think title, champion status, old-god baggage, and divine identity politics.
- Why it matters: many of Donut's later labels and powers only make sense if you remember Nekhebit is not just decorative lore.

### Nothing Special

- Type: enriched pet biscuit variant.
- First major appearance: book 8.
- What to remember: turns a creature into a dungeon pet and reverts it to juvenile form.
- Why it matters: basic-sounding name, major transformation consequences.

### Nothing Special Party Companion

- Type: enriched pet biscuit variant.
- First major appearance: book 8.
- What to remember: similar transformation logic to Nothing Special, but more relational and party-tied.
- Why it matters: this is one of the variants where the coercive side of crawl pet mechanics becomes especially obvious.

### Oozy Form

- Type: spellbook spell.
- First major appearance: book 8.
- What to remember: defensive and camouflage-heavy slime-form spell with movement overlap.
- Why it matters: it enters the same tactical category as other phase/intangibility effects.

### Paramour

- Type: enriched pet biscuit branch.
- First major appearance: book 8.
- What to remember: a relationship-coded biscuit effect.
- Why it matters: it is one of the clearest examples of the system turning intimacy and dependency into mechanics.

### Pet Biscuits

- Type: base item family.
- First major appearance: book 1.
- What to remember: the foundational pet-item line from which the stranger enhanced variants evolve.
- Why it matters: if you remember ordinary pet biscuits, the more exotic biscuit branches make much more sense.

### Petey Motteux's Little Secret

- Type: niche ring item.
- First major appearance: book 8.
- What to remember: strange, specific, and extremely Carl.
- Why it matters: it represents the long-running pattern where obscure inventory becomes decisive when Carl combines it with a larger plan.

### Puddle Jumper

- Type: Donut mobility spell.
- First major appearance: pre-book-8; explicitly discussed in book 8.
- What to remember: one of Donut's mature movement options.
- Why it matters: book 8 treats it as established enough to discuss level breakpoints and tactical value.

### Radar

- Type: utility/search skill.
- First major appearance: referenced in book 8.
- What to remember: part of the race-floor search toolkit.
- Why it matters: another example of practical information tech deciding outcomes.

### Requite Dahlia

- Type: class-linked flower package.
- First major appearance: book 8.
- What to remember: associated with Donut's broader mercenary or support-gear uplift package.
- Why it matters: useful mainly as a marker that Donut's class choices now affect whole crews, not just herself.

### Ring of Divine Suffering

- Type: Carl artifact ring.
- First major appearance: book 3.
- What to remember: Carl's pain-for-power ring, scaling through suffering and carrying both benefit and cost.
- Why it matters: one of the most emblematic items in Carl's entire build.

### Scorpion's Bite

- Type: Donut weapon mount.
- First major appearance: book 8.
- What to remember: gives Donut a way to deploy ranged gear and named bolts effectively.
- Why it matters: solves the obvious logistical problem of "how is the cat using crossbow hardware right now?"

### Sekhmet

- Type: goddess / class context.
- First major appearance: book 8 package context.
- What to remember: the name signals feral, court-linked, aggressive divine power in Donut's current setup.
- Why it matters: if a book-8 item mentions Sekhmet, it is usually part of the same Donut class ecosystem.

### Shield Spell

- Type: defensive spell.
- First major appearance: pre-book-8; still used in book 8.
- What to remember: practical survival magic, especially in movement-heavy or debris-heavy situations.
- Why it matters: included because book 8 repeatedly rewards boring competence.

### Smoke Form

- Type: Donut class-linked mobility/phase spell.
- First major appearance: book 8.
- What to remember: a phase-style or incorporeality-style spell from the Assassin of Sekhmet package.
- Why it matters: it is good enough that the book directly compares it to other high-value movement effects.

### Source Scroll / Water Source Scroll

- Type: utility scroll.
- First major appearance: book 8.
- What to remember: low-glamour, high-utility environmental tool.
- Why it matters: this is the kind of scroll that matters in Dungeon Crawler Carl because someone uses it intelligently, not because it sounds legendary.

### Spellbook Of The Floor

- Type: recurring reward category.
- First major appearance: pre-book-8, referenced again in book 8.
- What to remember: one of the reliable ways the series hands out new magical options.
- Why it matters: when a spellbook is mentioned, it often marks the arrival of a new tactical lane rather than just another stat upgrade.

### Split Personality

- Type: spell.
- First major appearance: book 8 discussion.
- What to remember: a spell that cuts a target in two in a personality or identity sense.
- Why it matters: included because book 8 is unusually interested in identity splits, domination, and fractured selves.

### Steady Charge

- Type: buff skill.
- First major appearance: book 8.
- What to remember: one of the support boosts granted through Donut's class-linked gear packages.
- Why it matters: again, Donut's current build is about uplifting allied bodies on the field, not just herself.

### Torch

- Type: Donut utility spell.
- First major appearance: book 1.
- What to remember: basic light spell, but it has never stopped being useful.
- Why it matters: a good example of how older low-tier utility remains relevant deep into the series.

### Transformation Spell

- Type: hostile effect category.
- First major appearance: earlier in the series, explicitly relevant again in book 8.
- What to remember: transformation effects are not cosmetic in this world. They can radically change tactics, identity, and survivability.
- Why it matters: book 8 is full of mechanics that punish anyone who treats body-state as stable.

### Twinkle Toes

- Type: Donut movement spell.
- First major appearance: earlier than book 8.
- What to remember: one of Donut's standard movement or pet-mobility tools.
- Why it matters: book 8 invokes it partly to show that old solutions do not automatically apply to new floor constraints.

### Wall of Fire

- Type: Donut offensive/control spell.
- First major appearance: pre-book-8; explicitly escalated in book 8.
- What to remember: by the time book 8 discusses it, this is no longer a modest fire spell.
- Why it matters: one of Donut's best examples of "simple spell, terrifying at high level."

### Wrap of Sekhmet's Compassion

- Type: pet headband.
- First major appearance: book 8.
- What to remember: healing and resurrection-adjacent support headband from the Sekhmet package.
- Why it matters: proves again that Donut's current package is a whole team toolkit.

### Ysalte

- Type: goddess / memorial-crystal context.
- First major appearance: book 6.
- What to remember: one of the divine figures whose death helps clarify how memorial crystals work.
- Why it matters: useful as a reference point for the crystal system and the instability of the divine order.

## Best Quick-Recall Cluster

- If you only want the ten most useful entries before reading book 8, review:
- [`Gate of the Feral Gods`](#gate-of-the-feral-gods)
- [`Ring of Divine Suffering`](#ring-of-divine-suffering)
- [`Memorial Crystal`](#memorial-crystal)
- [`Memorial Crystal: Apito`](#memorial-crystal-apito)
- [`Emberus Ring`](#emberus-ring)
- [`Hole Spell`](#hole-spell)
- [`Gloom Wraith Phase`](#gloom-wraith-phase)
- [`Enhanced Pet Biscuits`](#enhanced-pet-biscuits)
- [`Nothing Special Party Companion`](#nothing-special-party-companion)
- [`Assassin of Sekhmet`](#assassin-of-sekhmet)

## One-Paragraph Memory Hook

- If book 8 mentions a named artifact or spell and it feels familiar but slippery, it probably belongs to one of four groups: Carl's suffering-and-survival gear, Donut's title/class packages, divine relics like memorial crystals, or transformation/control mechanics such as biscuit variants. That framework will correctly place most of the recurring magical references you are likely to hit.
